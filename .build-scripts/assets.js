...
const moduleRules = [
    ...
    {
        test: /\.(png|jpe?g|gif|svg)$/i, // Matches image files (png, jpg, gif, svg)
        use: [
            {
                loader: 'file-loader', // Copies image files to the output folder
                options: {
...
module.exports = (env, argv) => {
    ...
    const config = {
        ...
        watch: buildMode === 'development', //Turn on watch mode. This means that after the initial build, webpack will continue to watch for changes in any of the resolved files.
        watchOptions: { //
